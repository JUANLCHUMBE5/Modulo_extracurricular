...
// Inicialización diferida del pool de conexiones
let pool: pg.Pool | null = null;

export function isPgEnabled(): boolean {
  return process.env.DATA_MODE === "postgres" || !!process.env.DATABASE_URL;
}

function getPool(): pg.Pool {
  if (!pool) {
    const connectionString = process.env.DATABASE_URL || "postgresql://postgres:postgres@localhost:5432/extracurricular";
    pool = new Pool({
      connectionString,
