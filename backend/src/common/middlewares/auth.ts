...
import { getDb } from "../../database/local/dbLocal.js";
...
export function requireRole(allowedRoles: string[]) {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    // 2. Si el rol pertenece al portal de padres o el array de roles contiene padres, valida el acceso directo
    const reqRoles = Array.isArray(req.user.roles) ? req.user.roles : (req.user.role ? [req.user.role] : []);
    if (reqRoles.includes("padres")) {
    ...
    try {
      ...
      // 5. Mapea la nomenclatura de roles internos a roles del middleware (Array de roles soportado)
      ...
      const isAdministrator = mappedRoles.includes("administrador");
      const hasRole = allowedRoles.map(r => String(r).toLowerCase()).some(allowed => mappedRoles.includes(allowed));
      ...
      if (isAdministrator || hasRole || hasSpecificPermission) {
        req.user.role = mappedRoles[0] || "usuario";
