...
export async function enviarCorreoGenerico({ para, asunto, html, adjuntos = [] }: EnviarCorreoParams): Promise<EnviarCorreoResult> {
  ...
  if (transporter) {
    try {
      ...
      const info = await transporter.sendMail({
        from: remitente,
        to: para.trim(),
        bcc: process.env.EMAIL_COBRANZAS || undefined, // Envío automático de copia oculta a cobranzas
        subject: asunto,
        html: html,
  ...
  try {
    ...
    const safeSubject = asunto.replace(/[^a-zA-Z0-9]/g, "_").slice(0, 30);
    const fileName = `${Date.now()}_para_${para.replace(/[^a-zA-Z0-9]/g, "_")}_${safeSubject}.html`;
    const filePath = path.join(tempDir, fileName);
    fs.writeFileSync(filePath, html, "utf8");
    console.log(`[MAIL MOCK] Correo HTML guardado localmente en: ${filePath}`);
  } catch (fsError: any) {
    console.error("[MAIL MOCK ERROR] No se pudo guardar el correo local:", fsError.message);
  }
