...
import { CoordinacionService } from "../services/coordinacion.service.js";
...
export class CoordinacionController {
  ...
  async previsualizarCargaExcel(req: Request, res: Response): Promise<void> {
    try {
      const { parseJsonArray, parseJsonObject, normalizarPeriodo } = await import("../../../infrastructure/files/file.service.js");
      const { generarPreviewCargaExcel } = await import("../../../infrastructure/excel/excel.service.js");
      ...
      const dbProgramasMap = new Map((db.programas || []).map((p: any) => [p.id, p]));
