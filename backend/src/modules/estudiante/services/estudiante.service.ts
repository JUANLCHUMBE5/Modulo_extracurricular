...
import { limpiarDni as limpiarDniHelper } from "../../../infrastructure/files/file.service.js";
...
export class EstudianteService {
  ...
  async getEstudiantes(page: number | null, limit: number) {
    const db = await estudianteRepository.getDb();
  ...
  async getEstudianteSecretaria(dni: string, periodo: string) {
    ...
    let esExterno = false;
    if (!student) {
      const extStudents = await estudianteRepository.readExternalStudents();
      if (extStudents[dni]) {
