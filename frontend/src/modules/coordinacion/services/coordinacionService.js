...
export async function registrarAlumnoIndividualCarga({ periodo, programaId, dni, nombre, grado, forzarGrado = false }) {
  ...
  if (programa.estado && (String(programa.estado).toLowerCase() === "finalizado" || String(programa.estado).toLowerCase() === "archivado")) {
    errores.push(`El programa ${programa.nombre || "seleccionado"} está ${programa.estado}. No se pueden registrar alumnos en programas finalizados o archivados.`);
  }
