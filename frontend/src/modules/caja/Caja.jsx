...
export default function Caja({
  ...
  const [historialAlumnoRegistro, setHistorialAlumnoRegistro] = useState(null);
  ...
  async function abrirHistorialAlumno(fila) {
    if (!fila?.dniEstudiante) {
      toast.error("Historial del alumno", { description: "No se encontro el DNI del estudiante." });
      return;
    }

    setHistorialAlumnoRegistro(fila);
    setHistorialAlumno([]);
    setHistorialAlumnoAbierto(true);
    setHistorialAlumnoCargando(true);

    try {
      const pagosHistoricos = await obtenerHistorialAlumnoCaja(fila.dniEstudiante);
      const filasReporteAlumno = reporteCaja
        .filter((item) => item.dniEstudiante === fila.dniEstudiante)
        .map((item) => ({ ...item, origenHistorial: "reporte" }));
      const vistos = new Set();
      const historialCombinado = [...filasReporteAlumno, ...pagosHistoricos]
        .filter((item) => {
          const key = item.pagoId || item.id || item.inscripcionId || `${item.periodo || ""}-${item.programaId || item.programa || ""}-${item.fecha || item.fechaPago || item.fechaRegistro || ""}`;
      ...
      toast.error("Historial del alumno", { description: error.message || "No se pudo cargar el historial." });
    } finally {
      setHistorialAlumnoCargando(false);
    }
  }

  function cerrarHistorialAlumno() {
    setHistorialAlumnoAbierto(false);
    setHistorialAlumnoRegistro(null);
    setHistorialAlumno([]);
    setHistorialAlumnoCargando(false);
  }
          ...
          <>
            {vista === "reportes" ? (
              <CajaReportes
                ...
                historialAlumnoAbierto={historialAlumnoAbierto}
                historialAlumnoCargando={historialAlumnoCargando}
                historialAlumnoRegistro={historialAlumnoRegistro}
                opcionesReporte={{
                  ...opcionesReporte,
